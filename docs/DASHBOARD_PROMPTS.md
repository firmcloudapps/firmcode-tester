...
Read docs/DASHBOARD_DESIGN.md, docs/PRD.md, docs/TASKS.md, CLAUDE.md, and docs/REFERENCE_ANALYSIS.md. Design the Firmcode dashboard visual system for a clean modern light-mode CodeRabbit-style code review SaaS. Use Next.js, TypeScript, and Tailwind CSS. Define Tailwind theme tokens, layout primitives, typography, status colors, severity badges, cards, tables, tabs, drawers, tooltips, buttons, form controls, code snippets, and empty/loading/error states. InsForge owns authentication, account, member, workspace, and billing UI entry points, and NeonDB/PostgreSQL backs tenant-scoped application data. Include required GitHub OAuth account connection and GitHub App installation as first-class setup states. Do not build a landing page. Produce implementation-ready component notes and acceptance criteria.
```
...
Build the Firmcode PR Review workspace as the focused page for repository review automation, using the attached developer-dashboard reference as structural inspiration: left navigation, PR Review active state, provider tabs, GitHub OAuth/App cards, compact repository rows, enabled toggles, settings icons, and Run actions. Use Firmcode branding and tokens rather than copying the reference brand. Every signed-in Developer can connect GitHub OAuth, add/sync repositories, enable automation, run/retry reviews, and track report analysis. Admins additionally manage billing, member access, global workspace settings, and support/safety controls. Do not expose raw OAuth tokens, installation tokens, private keys, webhook secrets, or raw payloads. Role-gate mutating controls according to docs/AUTHORIZATION.md.
```
...
Build the Firmcode GitHub OAuth plus GitHub App setup and sync dashboard flow. Implement /github/installations as a InsForge-authenticated setup/status page with required GitHub OAuth account connection, GitHub App install entry point, missing-config state, installation status, repository sync status, and safe error/retry states. OAuth start and callback routes must require a signed-in InsForge user and resolved workspace membership. Wire Connect GitHub, Settings GitHub App actions, repository Sync GitHub, and row sync controls to real routes/APIs or disabled planned states. Do not expose private keys, webhook secrets, OAuth access tokens, installation tokens, or raw payloads. Use the full-width light dashboard layout and brand tokens from docs/DASHBOARD_DESIGN.md.
```
...
Acceptance/test criteria: Add or update route manifest/component tests so active dashboard links and primary actions point only to implemented internal routes or external InsForge/GitHub URLs; verify planned actions are disabled and accessible; do not declare done until checks pass or any inability to run them is documented.
