...
- The endpoint is now behind the shared Clerk dashboard auth guard. User identity comes from the verified Clerk token; `x-firmcode-user-id` is rejected and `x-firmcode-workspace-id` is only an optional selector after membership verification.
