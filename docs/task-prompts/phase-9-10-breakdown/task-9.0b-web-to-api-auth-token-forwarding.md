# Task 9.0b: Web-To-API InsForge Token Forwarding
...
Replace env/header-shim authentication in the Next.js dashboard with InsForge session-token forwarding. Server components and route handlers should read InsForge auth state with @insforge/sdk server helpers. Web-to-API calls must send Authorization: Bearer <InsForge session token> using the configured INSFORGE_BASE_URL/template. The user ID must not be sent from FIRMCODE_DASHBOARD_USER_ID in production.

Keep an explicit test/local bypass only where tests need stable fixtures, and name it clearly so it cannot be mistaken for production auth. If a workspace selector is still needed, pass only the workspace ID as a selector after InsForge authentication; never pass caller identity as a trusted header.
...
- Add tests that missing InsForge session yields a redirect or 401-safe state before calling the API.
- Add tests that mutation proxy routes forward InsForge bearer tokens.
- Add tests that GitHub OAuth start/callback routes require a signed-in InsForge session.
...
- Production request creation no longer reads FIRMCODE_DASHBOARD_USER_ID as caller identity.
- x-firmcode-user-id is not emitted by production web code.
- GitHub OAuth start and callback bind to the signed-in InsForge session.
