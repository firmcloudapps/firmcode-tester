...
Implement Clerk-to-Firmcode workspace and membership resolution. For Clerk Organizations, map each Clerk organization to one workspaces row by clerk_org_id and map organization memberships to workspace_memberships. Without Clerk Organizations, create or resolve a personal workspace for the Clerk user and give that user Developer role by default.

Define role mapping clearly:
- explicit Firmcode role metadata wins when configured
- Clerk org owner/admin maps to Admin
