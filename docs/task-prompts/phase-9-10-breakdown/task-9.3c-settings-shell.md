...
Code context requirement: Before implementing, inspect existing settings routes, dashboard shell, Clerk organization/user patterns, configuration APIs, tests, and Tailwind form controls.

Build or complete the Settings shell with tabs for General, GitHub App, Members, API Keys, Data Retention, and Notifications. Use Clerk-owned identity and member management entry points where available, and keep Firmcode-owned settings focused on GitHub installation mapping, repository review configuration entry points, retention policy display/config, notifications placeholders, and API key placeholders if implementation is not ready.

The shell should be Clerk-authenticated and role-aware according to docs/AUTHORIZATION.md. Sensitive settings should be visibly disabled or hidden for roles that cannot modify them, while preserving useful read-only context for lower roles where appropriate.
...
- Add Clerk-gated access tests and role-based visible/disabled state tests for sensitive settings.
...
- Clerk-owned functionality is linked or delegated instead of reimplemented.
